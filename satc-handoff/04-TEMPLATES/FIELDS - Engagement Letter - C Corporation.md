...
> **Derived from the business engagement letter on 26 August 2026.** The firm:
> *"let's go separate - there may be other things we want specific to them anyway."*
> Before that, a C corporation could not be sent an engagement letter at all: the
> business letter's section 02 is entirely about Schedules K-1 and merges
> `<<ScheduleK1Target>>`, a date the interview correctly never asks a C corporation
> for — so the render **refused**. This letter is that one with everything untrue of
> a C corporation removed and **nothing invented to replace it**. The template's
> on-screen `.ref` block lists all six removals and why each is untrue.
> 
> **Sections renumbered 01–09**, with no gap — a signed document must not skip a
> clause number.

Companion to `SATC Engagement Letter - C Corporation.html`. The same table renders on screen below the letter itself (`@media print` hidden, so it never reaches a client).
...
**Total: 25 fields + 1 flag. No repeating lists.**
