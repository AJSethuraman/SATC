...
import os
# the build under walk: commit a5a8aa2, frozen with `git archive` into scratch (CUBE_SRC) so another session
...
def shot(name, whole=False):
    for _ in range(5):
        root.update(); time.sleep(0.1)
    # the window alone, as the person sees it (the virtual screen is bigger than the window); whole=True
    # photographs the whole virtual screen, for the file dialog, which is a window of its own
    if whole:
