...
channel and asset-class plants unchanged). Runs the split the walk ran on both (commit a5a8aa2: minimums applied, rate ratio first)
...
raw = {"name": "noplant", "schema_version": 1, "columns_confirmed": True,
       ...
       "benchmark": {"min_units": 30, "min_events": 10, "worse_at": 1.25, "better_at": 0.8, "confidence": 0.95,
                     "power": 0.8, "compare_to": "peers", "many_tests": "bh", "materiality": "1% of losses", "revenue_line": "luck"},
...
for label, path in books.items():
    ...
    for g in res.grids:
        ...
        print(f"{label:8} {g.band:8} x {g.dimension:11} bad-rate ratio {p['ratio']:.2f} "
              f"({p['ratio_lo']:.2f} to {p['ratio_hi']:.2f}), luck alone {p['ratio_p']:.2g}  odds {p['odds']:.2f}  "
              f"high half worse in {p['high_worse']} of {p['pockets']}")
    print("   split moves with:", {k: round(v, 2) for k, v in res.split_moves_with.items()})
