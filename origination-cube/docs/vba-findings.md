...
| 7 | Warnings for a missing key or a missing domain measure went to `Debug.Print`, which nobody sees | **Fixed.** Every warning is printed at the top of the output under WARNINGS. Since ruling OC-14, a missing key, GCO or RANR is no longer a warning: the run refuses | `test_finding_7_*` |
