...
muts = [
 ...
 ("materiality ignored", E, '                s.material = s.dollars > 0 and s.dollars >= mat', '                s.material = s.dollars > 0',
  "materiality_as"),
 ...
 ("dollars against the book", B, 'f"=IF(judged_band,{sr(live.P_EX_BAND)},{sr(live.P_EX_BOOK)})",',
  'f"={sr(live.P_EX_BOOK)}",', "dollars_agree or engines_and_the_other"),
 ...
 ("luck side coloured", B, 'ws.cell(row=r, column=LVR_HIDDEN + j, value=f"={at(k, live.P_FLAG)}")',
  'ws.cell(row=r, column=LVR_HIDDEN + j, value=f\'=SUBSTITUTE({at(k, live.P_FLAG)},", not significant","")\')',
  "luck_gap_keeps"),
 ...
 ("three-way red on unheld rows", B,
  ...
  """    red = worse""", "three_way_rows_say"),
 ("luck not marked", B, '"{more} (not \'\n                            f\'significant)",IF(',
  '"{more}",\'\n                            f\'IF(', "luck_gap_keeps"),
 ...
 # 26 Sep 2026: one comparison decides the verdict, the dollars and materiality; profit reads literally
 # (tests/test_one_comparison.py)
 ("dollars over the book under the band", E, '                        s.by_band, s.dollars = True, s.excess_band',
  '                        s.by_band, s.dollars = True, s.excess', "one_comparison"),
 ("band dollars against the whole band", E,
  '                    s.excess_band = (s.num - rb * s.den) if hi == "worse" else (rb * s.den - s.num)',
  '                    s.excess_band = (s.num - _rate(cells[(b, ALL)].rates[m.name].sums()) * s.den) if hi == "worse" '
  'else (_rate(cells[(b, ALL)].rates[m.name].sums()) * s.den - s.num)', "by_hand"),
 ("materiality on the book's dollars", E, '            if mat is not None and s.dollars is not None:\n                s.material = s.dollars > 0 and s.dollars >= mat',
  '            if mat is not None and s.excess is not None:\n                s.material = s.excess > 0 and s.excess >= mat',
  "one_comparison"),
 ("ranked by the book's dollars", B, '                if s.dollars is not None and s.dollars > 0:\n                    found.append((s.dollars, g, bl, dl, s))',
  '                if s.excess is not None and s.excess > 0:\n                    found.append((s.excess, g, bl, dl, s))',
  "one_comparison"),
 ("the reference dollars hidden", B, 'f"=IF(judged_band,{sr(live.P_EX_BOOK)},{sr(live.P_EX_BAND)})"]',
  '\'=""\']', "one_comparison"),
 ("Check silent on what decides", B, '        rows.append(("Decides each pocket", decides(res)))', '        pass',
  "one_comparison"),
 ("command line ranks by the book", "src/origination_cube/cli.py",
 ...
 ("literal gap from the other comparison", E,
  ...
  '        return literal(s.flag, s.vs_rest, s.dollars, "its band", line, s.p_band)', "one_comparison"),
 ("short and ahead swapped", E, '    side = "short of" if word in (WORSE, UNSURE_WORSE) else "ahead of"',
  '    side = "ahead of" if word in (WORSE, UNSURE_WORSE) else "short of"', "one_comparison"),
 ("not significant dropped from the literal reading", E,
