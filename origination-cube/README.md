...
```
pytest -q                          # 170 tests: one per finding, every Control answer applied, the workbook route, the split, the launcher
python tools/mutation_check.py     # puts 39 bugs back (the VBA's and today's rules); every one must be caught
```
