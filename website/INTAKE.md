...
| 2026-08-14 | **Removed `privacy.html` and `terms.html`.** The engagement-letter acknowledgement stays on the consent step, without the links. | The site is deliberately barebones for now. **Flagged and deferred, not resolved:** the form collects personal financial context and routes it through a third-party processor, so a privacy notice is worth restoring before the site takes real volume. |
| 2026-08-14 | **Trimmed to 11 steps.** Cut `rental_count`, `entity_count`, `headcount`, `contractor_count`, `states_detail`, `unfiled_years`, `transaction_volume`. | All were "how many / which one" detail. The *flag* is what scopes an engagement — that they have rentals, that years are unfiled, that it is multistate. The number is a five-second question on the call. Worst-case path 18 → 11; business tax + bookkeeping 11 → 9. |
| 2026-08-14 | **Urgency is now "Is this urgent?"** — three options, down from ten. | "What's driving the timing?" was a menu, not a question. What we need is whether it goes to the top of the pile; the reason surfaces in the first minute of the call, and services + status already imply most of it. The date follow-up now hangs off `urgency === 'deadline'`. |
| 2026-08-13 | **`business_structure` is multi-select.** "More than one business or entity" no longer competes with the structure options. | It is a *count*, not a structure. As a single choice it forced someone with two S corps to say either "S corporation" or "multiple entities" and lose the other, and made a Schedule C alongside an S corp — common — inexpressible. `hasBusiness()` now treats "not set up yet" as meaning no business only when it is the *only* pick. |
| 2026-08-13 | **Answers render as chips**, with an explicit "Select all that apply" / "Choose one" hint. | Requested. Each chip is still a real checkbox/radio hidden inside its label — buttons with `aria-pressed` would look the same and lose checked-state announcement, radio arrow-keys, and the `input:checked` reads in `collect()`. The hint replaces the square-vs-circle cue chips discard. |
