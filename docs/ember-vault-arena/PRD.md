...
2. [P0] The Ember Vault world, hand-authored, grown to about sixteen locations in a registry the engine reads (rooms, adjacency, props, items, monsters, talker start positions, objective sites). The map never varies by seed. The seed places loot within its declared sites, rolls every die, and orders initiative. **Built 19 September 2026 (`arena/world.py`, `ember-vault-world-1.0`, the session's drawing, every name and edge reversible by the firm, docket D13):** sixteen locations, July's five kept first and unchanged in shape, then the outer ring off the Threshold (Bell Tower, Drowned Cellar, Lantern Walk), the deep west off the Ironwood Gate (Root Hollow, Sap Works), the one Long Gallery joining west to east with the Undercroft under its hatch, the deep east off the Ossuary Gate (Reliquary, Charnel Stair, Bone Well) and the Parapet beyond the Egress. Five monsters (a Cellar Drowner and a Charnel Hound join July's three), eight caches, the Marrow Reliquary on the Reliquary's altar (loot 10). `rules`, `grid`, `combat`, `memory` and the board page take rooms, grids, props, reactions, monsters, caches, floor items, the schedule and the layout from the registry, which validates itself at import (symmetric adjacency, a door both ways on the floor, features and spawns on the floor, guardians in their rooms, known items, the schedule inside act IV with every room still open at the end reachable from the Vault, no two rooms overlapping on the board). Contraction seals the ring and the deeps from the outside in over rounds 37–47, two rooms on each of the last two rounds, so round 48 is played in the Vault, the Egress and the Parapet. **Not yet:** seeded loot placement within declared sites (contents are fixed per cache) and talker start positions (with item 25).
3. [P0] About twelve live objectives at any round of acts I–III, each individually pursuable, at least three requiring two or three characters to act in the same location in the same round. Value is heterogeneous by build and by secret objective so that trade exists. Exact counts are tuned on the mock simulator (seam 6) to the metrics in §8. **Partly built 19 September 2026:** the individually pursuable ones are on the map (two seals, eight caches worth first-find points and items of unequal value to a hoarder or a fighter, five monsters for a hunter, the relic for a hoarder, both gates for a lorekeeper, the Crown for everyone). **Built 19 September 2026, the cooperative sites (`world.SITES`, three):** the Gallery's braziers (two hands, +4 each), the Bone Well's winch (two hands, +3 each), the Bell Tower's bell (three hands, +5 each). Each hand is a feature of the room and an `interact` target while the site waits and no monster holds the room; the site wakes at upkeep only when every hand was lent by a different character in that round (`site_hand` per hand, `site_done` per party, `site_done` scoring category), and a round with too few hands is `site_lapsed` in public with the try kept in the next digest (`room.sites[].last_attempt`) so a brain can see who reached for what. The mock takes the hand matching its rank among the characters present, so two or three mocks cover a site without a word; the seed-52 house match rings the bell in round 2. Points are the session's, for item 5 to tune. **Still open:** whether twelve live objectives is what the map now offers (two seals, eight caches, five monsters, the relic, three sites, the Crown: seventeen sites of value, of which the caches empty and the sites wake once), to be counted on the simulator.
...
**The world registry.** One file the engine reads for rooms, adjacency, props, items, monsters, talker starts and objective sites, extending July's `ROOM_PROPS` and item registry pattern (the observation builder and the resolver read the same registry, so no effect can apply undisclosed). Authored by hand; the seed places loot only within declared sites. Built 19 September 2026 as `arena/world.py` with a validator at import and `tests/test_world.py`; the map view now publishes each room's cache and whether it has been found, and the mock hunts the nearest unfound cache with half its party so the whole map is walked under test.
