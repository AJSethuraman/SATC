...
| **B8** | **THE DISK IS NOT ENCRYPTED — measured 4 September 2026, and now a fact rather than an unknown.** `manage-bde -status C:` on the 930.52 GB OS volume: *Conversion Status: Fully Decrypted · Percentage Encrypted: 0.0% · Protection Status: Protection Off · Key Protectors: None Found.* | The drive holding `satc_vault.db` and `vault.key` is readable to anyone who takes it out of this machine or boots the machine from a USB stick. **The Windows account is not a control against that** — it is enforced by Windows while Windows is running, and a disk in someone else's hand is not running Windows. This is the second control B11 leans on: the argument for not building MFA is physical possession of the machine, and physical possession is the exact case unencrypted storage does not survive. The vault's own AES-256 field encryption still stands and is what keeps this short of plain data loss — but its key sits on the same unencrypted disk. | measured 4 Sep 2026 by the owner, `manage-bde -status C:`; bears on A4a, B11 |
...
| **B13** | **Nothing suspends disposal for a record under audit, examination or litigation.** *(New 4 Sep 2026.)* There is no legal hold — no way to mark a client, an engagement or an artifact as "do not destroy, a claim is foreseeable". The words "legal hold" and "litigation" appear nowhere in this document. | **This gap is dormant today and arms itself the moment §A9's disposal is built.** A disposal pass that honours the seven years and knows nothing about holds will destroy, on schedule and with a clean audit trail, exactly the records a claim needs — and destruction after a claim is foreseeable is a far worse position than never having had a schedule at all. **So the hold has to ship with the disposal, not after it.** The firm's own research already found this: `docs/research/tax-practice/04-critique.md:16` — *"Both the purge-on-close review-note rule and the retention-clock disposal queue can destroy records after a claim is foreseeable. Nothing suspends disposal."* | analysis at `04-critique.md:16` and `00-README.md:79`; design basis at `02-gap-analysis-and-roadmap.md:238-248`; §A9 |
| **B14** | **A copy of `vault.key` taken off this machine would not have worked, and would have looked like it had.** *(New 4 Sep 2026.)* `crypto._seal` calls `CryptProtectData` with no entropy and no machine flag (`crypto.py:64-73`), so the 296-byte file is bound to one Windows account on one machine. | Restored onto a replacement laptop after a disk failure it cannot be unwrapped — the one day it exists for. The firm hit this directly: asked to paste the file into a password manager, they reported it *"looks corrupted, values cant be read at all"*, which was the binary wrapper doing its job. **Closed by `satc_system/scripts/vault_key.py`**, which exports the 32-byte key inside the wrapper as 44 characters of base64, refuses to overwrite an existing key (a vault given the wrong key is unreadable for good), and is proved by a test that carries a key to a second location and decrypts data written before the move. **The owner confirmed on 4 September that the key is stored in Bitwarden.** | `scripts/vault_key.py`, `tests/test_vault_key_export.py`; W6 in the defect register |
...
- **ANSWERED 4 September 2026, from the letters rather than from anybody's recollection of them.** The seven years runs **from the end of the engagement**, and the letters say when that is (Tax Preparation §09, and the same clause in Business Return and C Corporation):

  > *"Either of us may end this engagement in writing at any time... Our engagement otherwise concludes when we deliver the completed returns to you or, for e-filed returns, when you sign the authorization and we transmit them — not when the return is accepted."*

  So three endings: **written notice** from either party at any time, which outranks everything; **signature plus transmission** for an e-filed return, needing both; **delivery** for anything not e-filed. Two things that are emphatically not endings: **acceptance**, which the letter rules out in as many words, and **payment**, which appears nowhere in the clause — fees for work done are a debt, not a duration.

  **Bookkeeping is a different contract.** Its letter has no "concludes when" clause at all, only *"either of us may end this engagement on `<<NoticePeriod>>` written notice"*. A rolling engagement concludes on nothing by itself, so written notice is its only ending and silence must mean keep.

  Implemented as `satc/retention.py`, which derives the date from facts somebody recorded and returns *undetermined* with a reason rather than guessing. **Gap B4 stands** — nothing destroys anything on a schedule — and **W8 is new**: written notice is recorded nowhere, so a bookkeeping engagement cannot yet acquire a disposal date at all.
...
- `[CONFIRM: **how disposal is suspended for a client under audit, examination or litigation** — who can place the hold, what it covers (one engagement, or everything for that client), who may lift it, and how a held record is proved to have been held. There is no such mechanism today (Gap B13) and none is needed until disposal exists — but the two must ship together, because a schedule that destroys on time and ignores holds is worse than no schedule.]`
