...
| **W8** | **Two of the three ways an engagement ends are recorded; the third is recorded nowhere.** The letters end an engagement on delivery (`Deliverable.delivered_on`), on signature-plus-transmission (`Filing.transmitted_at` plus the signed 8879 as a `ReceivedDocument`), **or on written notice from either party — and nothing in the schema holds that date.** | `retention.engagement_ended` reads all three and returns `Undetermined` when none is on file, so the gap is visible rather than silently treated as "not ended yet". It bites hardest on **bookkeeping**, whose letter has no "concludes when" clause at all: written notice is its ONLY ending, so a bookkeeping engagement can never acquire a disposal date until this is built. Needs a date and a basis on `Engagement`, a store column and a migration — the `engagement_ref` and `parts` pattern | open **GATED TO THE BOOKKEEPING LAUNCH, 4 Sep 2026.** The firm: *"note those as things to deal with when we are ready to market the bookkeeping officially."* Deferred deliberately, not forgotten — and the deferral is safe in one direction only. Nothing is destroyed today, so no record is at risk; what is at risk is **selling a bookkeeping engagement whose retention promise the software cannot keep**. A tax engagement ends by itself and acquires a disposal date on its own; a bookkeeping engagement never does. **Before bookkeeping is marketed:** record written notice (a date and a basis on `Engagement`, a store column, a migration), and answer the question the notice period raises: the bookkeeping letter ends the engagement on **`<<NoticePeriod>>` written notice**, a per-engagement field (`registry/fields.yaml`, `source: bookkeeping`; the spec's example is *thirty (30) days*). So does the engagement end on the date notice is GIVEN, or when the period EXPIRES? The letter also says the firm will *"complete the period in progress"*, which points at the later date — and the two can be a month apart, which is a month of the seven-year promise. Nothing answers it today. |
...
## Found by running the software, 4 September 2026 (evening)

Two agents were asked the same question — *can a person run this end to end,
and can an agent* — and told to work from source. The findings below are the
ones **reproduced here afterwards**, independently, because the register's own
rule is that a defect is listed once it has been reproduced. The rest of what
those reports contain is a set of decisions for the firm, not defects, and went
to the docket instead.

| # | What | State |
|---|---|---|
| **W9** | **`--store` isolates the files and not the money.** `cli.py invoice` builds a Square payment link through `payments.link_for(fields, using=payments.processor(sandbox=args.sandbox))` (`client-documents/cli.py:618-621`). `--store` routes only where the invoice JSON is written; the processor and its token come from `registry/payments.yaml` and the Windows credential store, so **a run pointed entirely at a temp store still reaches the firm's production Square account** unless `--no-link` or `--sandbox` is typed. An agent told "point tests at a temp store" — the standing instruction on this machine — would reasonably believe it had isolated itself, and would be wrong. The idempotency key is `f"satc-{invoice}"` (`payments.py:301`) while the invoice number is scanned from the **local** store only (`invoicing.py:59-70`), so a fresh temp store starts at `2026-0001` and collides with keys already used on the live account | **open, and it is the most serious thing found today.** Reproduced here only by reading the call path; **no Square call was made from this session.** One was made by the assessment agent, which reported back `400 — This idempotency key has already been used to create a Payment Link` and therefore created nothing. The reason a 400 is not reassurance: 400 is what a *differing body* returns. A matching amount returns the **existing link** — which would hand a fictional test client a real client's payment link. **FIXED 4 Sep 2026 (PR #255).** The firm chose the default: *"--no-link defaults on any non-default --store"*. `payments.link_follows_the_store` decides once, at the seam -- `--no-link` never, `--link` always, otherwise the default store gets a link and no other store does, and the suppression is printed rather than silent. `--link` had to exist: a safety default with no override is a wall, not a default. **The test was decoration on the first attempt** -- it watched the HTTP transport, which `processor()` never reaches when no token is configured, and asserted on wording the *old* code also printed. It passed against the defect it was written to catch, and only the mutation run found that |
| **W10** | **A script with no human in it is "the owner".** `acting_actor()` ends `return Actor.owner() if has_request_context() else Actor.system("headless")` (`satc_system/src/satc/app/state.py:66`). Its own docstring says the opposite two lines above: *"Anything else — a script, a scheduled sweep, an API tool, a model rung — gets a system actor and is refused by require_human at the gate."* **A script is exactly what it does not catch**, because `app.test_client()` and `test_request_context()` both create a request context. The gate reads the **transport**, not a principal | **open. Reproduced here 4 Sep 2026**, independently of the agent that raised it: inside `app.test_request_context("/")`, `acting_actor()` returns `Actor(kind='human', name='owner')` and `require_human` passes; outside one it correctly returns `Actor(kind='system', name='headless')`. The closed `Actor` set, the sticky provenance and the refusal to auto-confirm a model-produced value are all sound — **the principle is right and one line of the derivation is wrong.** The Forge's own rule 7a already names the shape of the fix (a role and an assignment set by whatever launches the process, never by the caller). **FIXED 4 Sep 2026.** The firm: *"Adopt the Occam shape -- launcher-set role and assignment."* `satc/principals.py` ports it: capabilities named by KIND OF HARM rather than per-endpoint, roles as capability sets, assignment as client globs, all set by whatever launches the process. `acting_actor()` asks the principal instead of asking Flask whether a request exists, so all sixteen existing `require_human` choke points refuse an agent without one of them being touched. `ai_staff` cannot confirm a staged value, issue an invoice, record a payment, change a price or contact a client; it keeps read, intake, client creation and engagement generation, because a role stripped past the job gets worked around. The MCP server now launches as `ai_staff` (`setdefault`, so an `observer` briefing is not promoted), and a test proves the engine still refuses the dangerous acts with writes fully enabled -- registration is a convenience now, not the boundary, and `docs/MCP.md` no longer claims otherwise. **ONE CASE IS DELIBERATELY NOT FIXED:** a local script that declares no role is still the owner -- the same call Occam made, because there is no authentication here, the tailnet is the perimeter, and a restrictive default only teaches everyone to pass `owner` everywhere. Written into the module docstring so it is not silently "improved" later |
| **W11** | **`SATC_DATA_DIR` was honoured by two callers out of eight, and one of the six was `reset`.** `_default_dir`'s docstring had always promised *"`SATC_DATA_DIR` (handled by the caller) always wins over this default"*; `satc.app.state` and `satc.doctor` handled it, and every `--dir`-taking CLI command read `DEFAULT_DIR` instead. Verified by running it: `SATC_DATA_DIR=<empty temp dir> satc chase` printed the **real** store's clients and wrote nothing to the temp one. `satc reset --yes` under the same variable would have deleted `build/data` — the live store — while the operator believed the run was scoped to scratch | **Fixed 4 Sep 2026.** One `resolve_dir()` at the choke point every store already passes through, precedence `--dir` → environment → default; `reset` and `doctor` call it too. 11 tests, including the delete-the-right-one case against a stand-in for the real store so nothing real is involved, a mutation test that reintroduces the old line and proves the new tests catch it, and a source check that no caller resolves the directory for itself again |
| **W12** | **467 errors in `satc_system` and 1,165 in `client-documents`, and none of it was the code.** `<temp>/pytest-of-<user>` on this machine carries a DACL that denies everything: `icacls` cannot read it back and PowerShell is refused identically. pytest cannot scan its own scratch root, so **every test that asks for `tmp_path` dies at setup**, before its first line runs. `client-documents` went from `1434 passed, 2 skipped` earlier the same day to zero passing | **Fixed 4 Sep 2026** in both suites by choosing a different scratch root — repairing the locked directory needs an elevated shell, which a test suite may not assume. `canon/conftest.py` had already hit this and worked out both traps: a root inside the repository makes every tree-walking guard read the suite's own fixtures (canon's client-data check reported a taxpayer identifier that was never there), and a long root breaks `git` with a 260-character path failure that mentions nothing about length. Copied rather than imported — canon lifts out whole. **`desk/` has no such handling and is presumably still failing this way**; it belongs to another session, so it was left alone and is reported instead. **A GREEN CI IS NOT EVIDENCE THIS IS FIXED.** Raised by the credit-suite session, 4 Sep 2026: CI runs on Linux, which never sees the DACL, so both the credit-suite and desk jobs went green today while the Windows runs were erroring. Anyone checking this off against CI is reading a machine that cannot reproduce it. A zero-code stopgap for a machine that hits it: `pytest --basetemp=<some writable dir>` |

| **W13** | **The product never created an `Engagement` at all.** Both assessment agents reported that `Engagement.engagement_ref` -- the join between the ref a client sees (`2026-0001`) and this system's key (`SATC-001000`), added on the firm's instruction *"ADD THE FIELD"* -- had no writer. Reading it here found something larger: **every use of `mart.engagements` in `src/` was a READ.** `rate_plan_for`, the comms screen, the intake fan-out, the workbook, the dashboard totals -- all readers. The only writers were `fixtures/synthetic.py` and the store's own loader, so an `Engagement` existed for the four demo clients and for nobody else. The field had no row to sit on, and neither did the rate plan agreed, the fee, invoiced, or paid | **Fixed 4 Sep 2026**, after the firm settled ownership: *"client-documents owns the engagement; satc_system holds the return."* `engagement_for(..., create=False)` is the one producer, idempotent by `(client_id, tax_year)`; only `create_engagement_from_intake` passes `create=True`, because that is the one call that means *this client has a contract for this year*. `POST /engagements/<job_id>/ref` records it, with the box on the engagement screen -- a route with no control on any page is a route nobody can reach, which was the whole defect. A duplicate ref is refused **at the keyboard**: `client_for_ref` returns nothing when a ref names two clients, correctly, but at collection time that refusal is indistinguishable from "nobody ever set it". `collect`'s message now names a place that exists rather than a field with no setter |
| **W14** | **An extension tells the client two things that cannot both be true.** `event --kind extension` supplies `MaterialsDeadline` -- "send everything by 1 August 2027" -- and changes nothing else. The engagement's original `FirstDeliverable` promise, 1 April 2027, stays where it was. So the pack promises delivery four months before it asks for the papers. The agreement check has always been able to see this; **nothing on the `event` path ever asked it**, so every extension notice this practice has produced went out over a self-contradicting record | **open — and it is the pre-send gate's SECOND real catch**, found 5 Sep 2026 on the day `event` was first put behind it. The first was `extension-is-not-more-time-to-pay`, which the notice states only as a heading while the checker stripped headings — fixed the same day at the firm's direction (*"compliance floors read headings; the other checks do not"*), because that exemption was written for words that must be ABSENT and was being applied to a sentence that must be PRESENT. This one is not a checker fault. **What an extension should do to the deliverable promise — move it, clear it, or ask — is process, not a thing to decide in a test file.** `tests/test_lifecycle.py` asserts the refusal rather than the fix, and one content test forces past it with a logged reason so the wording stays covered |

**W9 and W10 are the same shape, and it is the shape of the evening's whole
finding:** a control that reads *how* a call arrived rather than *who* made it.
`--store` reads the path and concludes the run is isolated; `acting_actor`
reads the request context and concludes there is a human. Both are true for the
case they were written against, and false for the case that now matters — an
agent driving the software the same way a person would.

**W11 and W12 are the same shape as each other:** a promise in a docstring and a
number in a report, each true when written and neither re-checked since.
