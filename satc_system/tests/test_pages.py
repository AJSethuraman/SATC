...
def test_intake_carries_the_page_all_the_way_into_the_workpaper(tmp_path, monkeypatch):
    ...
    # BY THE NAME THE FILE ARRIVED UNDER. This filtered on
    # `document_id == "IMG_4471.pdf"`, true when the id WAS the filename; it is
    # a content hash now (`doc-3b0c1d7007c8b017`), the change that killed a
    # silent data-loss bug where two files could share an id. The old filter
    # matched nothing, so this reported "staged nothing" while intake was
    # staging fine -- a test failing for a reason its name does not give (S15).
    staged = [f for d in getattr(state.gate, "documents", [])
              if d.display_name == "IMG_4471.pdf" for f in d.fields]
