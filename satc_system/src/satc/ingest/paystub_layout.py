"""Deterministic paystub layout extraction + click-to-teach profile reading.

Ported from the standalone ``twe`` tool. The flow, all on-device and AI-free:

1. :func:`extract_layout` renders a paystub (PDF/image) to a page image plus a
   list of words with normalized bounding boxes.
2. The preparer **teaches** a profile once by clicking which words hold each
   value; :func:`build_rules` turns those clicks into label-anchored rules
   (a region + the nearest text label).
3. :func:`apply_profile` reads any future stub of the same layout — label anchor
   first (robust to rows shifting), the taught region as fallback.

Only the layout is stored (regions + labels), never a client's amounts.
PDFs render with PyMuPDF; scanned images fall back to Tesseract OCR.
"""
...
# Job fields the form can be auto-filled with. (key, human label, kind)
TARGET_FIELDS: list[tuple[str, str, str]] = [
    ("gross_pay_per_period", "Gross pay this period", "currency"),
    ("federal_tax_withheld_per_period", "Federal tax withheld this period", "currency"),
    ("retirement_pretax_per_period", "Pre-tax retirement this period", "currency"),
    ("ytd_taxable_wages", "YTD wages", "currency"),
...
class FieldRule:
    ...
    def to_dict(self) -> dict:
        return {"field": self.field, "kind": self.kind,
                "region": list(self.region), "label_text": self.label_text}

    @classmethod
    def from_dict(cls, d: dict) -> "FieldRule":
        return cls(field=d["field"], kind=d.get("kind", _FIELD_KIND.get(d["field"], "currency")),
                   region=[float(v) for v in d["region"]], label_text=d.get("label_text", ""))
...
def _nearest_label(words: list[Word], region: list[float]) -> str:
    """The text label immediately left of a value region (its row, to the left)."""
    x0, y0, x1, y1 = region
...
def build_rules(words: list[Word], assignments: dict[str, list[int]]) -> list[FieldRule]:
    """Turn ``{field: [clicked word indices]}`` into deterministic field rules."""
    ...
    for fld, indices in assignments.items():
        ...
        rules.append(FieldRule(field=fld, kind=kind, region=region,
                               label_text=_nearest_label(words, region)))
    return rules
...
def _reconstruct_dropped_decimal(dollars: Word, numeric_sorted: list[Word]) -> str | None:
    ...
    if not _is_orphaned_cents(cents.text):
        return None
    return _parse_value(dollars.text + "." + cents.text.strip(), "currency")


def _read_by_label(words: list[Word], rule: FieldRule) -> str | None:
    """Read a value: the label anchor fixes the row, the taught region's x picks
    the column (so a label repeated across Current/YTD columns disambiguates)."""
...
class Profile:
    ...
    def to_dict(self) -> dict:
        return {"name": self.name, "pay_frequency": self.pay_frequency,
                "rules": [r.to_dict() for r in self.rules],
                "match_keywords": list(self.match_keywords)}

    @classmethod
    def from_dict(cls, d: dict) -> "Profile":
        return cls(name=d["name"], pay_frequency=d.get("pay_frequency"),
...
# ---------------------------------------------------------------------------
# Extraction (optional PyMuPDF / Tesseract stack)
# ---------------------------------------------------------------------------
...
def _load_pymupdf():
    try:
        ...
        try:
            ...
            raise PaystubError(
                "Reading a paystub file needs PyMuPDF. Install it with: pip install pymupdf"
            ) from exc
...
def _merge_number_fragments(items: list[list]) -> list[list]:
    """Stitch number tokens split apart by the renderer (e.g. ``6 , 653.85``)."""
    merged: list[list] = []
...
def extract_layout(data: bytes, media_type: str, *, dpi: int = 150) -> Layout:
    ...
    words = [Word(text=it[4], x0=it[0] / pw, y0=it[1] / ph, x1=it[2] / pw, y1=it[3] / ph)
             for it in items]

    pix = page.get_pixmap(dpi=dpi)
    png = pix.tobytes("png")
    return Layout(image_png_b64=base64.b64encode(png).decode("ascii"),
                  img_width=pix.width, img_height=pix.height, words=words)
...
def _ocr_words(page, pdf, *, dpi: int) -> list:
    ...
    try:
        ...
        raise PaystubError(
            "Reading image/scanned paystubs needs the Tesseract OCR engine installed and on "
            "your PATH. Install Tesseract, or upload a text-based PDF. Original error: " + str(exc)
        ) from exc
