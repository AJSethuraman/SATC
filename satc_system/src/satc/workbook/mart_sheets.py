...
def _table(ws: Worksheet, row: int, title: str, headers: list[str], rows: list[list],
           formats: list[str] | None = None, last_col_for_header: int = 12) -> int:
...
def build_data_mart_sheet(ws: Worksheet, mart: DataMart) -> None:
    S.paper_canvas(ws, max_col=13, max_row=120)
    X.set_widths(ws, {"A": 2, "B": 26, "C": 12, "D": 8, "E": 9, "F": 9,
                      "G": 14, "H": 14, "I": 13, "J": 13, "K": 13, "L": 12})
    ...
    row = _table(ws, row, "Return register", [
    ...
    row = _table(ws, row, "Line items (record-level, every field queryable)", [
    ...
    row = _table(ws, row, "Carryforward register (carried year to year)", [
    ...
    row = _table(ws, row, "Per-owner basis / capital rollforward", [
    ...
    row = _table(ws, row, "Estimated-payment history", [
    ...
    row = _table(ws, row, "Engagement & fees", [
