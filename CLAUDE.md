...
| `pogo-forge/` | **Personal, not practice-ops.** A local Pokémon GO roster and planning tool — cost engine, PvP rank tables, an appraisal-screenshot IV reader, batch triage. Unrelated to SATC's business; it lives here because it is the owner's. Worth reading for one habit: every number is either Niantic's own game master or is labelled, in the file, as not being. `HANDOFF.md` first | Python / FastAPI, SQLite | `cd pogo-forge && pytest -q` (**106 passed**, ~110s — 13 Sep 2026). Then run it: `python app.py`, port 8737 |
