...
| `schedule-c-pl/` | The free **Schedule C profit & loss tool** for satcllp.com — a sole trader's figures in, a P&L statement and a Schedule C worksheet out, as PDF and Excel. Browser-only, no backend, no analytics; the build **refuses** to write a page that would make a network request. Line labels are extracted from the official IRS PDF per year (`evidence/`) because the IRS **swapped lines 27a and 27b in 2025**. Read `DECISIONS.md` first. Source lives here; the page is **generated** into `website/tools/schedule-c-profit-and-loss/` | JavaScript (ES modules, no dependencies shipped), Python for the copy spec | `cd schedule-c-pl && npm test` (**130**), `npm run build`, `python3 copy.spec.py` (13), `npm run walk` — drives the built page in a real browser with the network cut off (35) |
