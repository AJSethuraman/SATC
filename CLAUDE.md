...
| `schedule-c-pl/` | The free **Schedule C profit & loss tool** for satcllp.com — a sole trader's figures in, a P&L statement and a Schedule C worksheet out, as PDF and Excel. Browser-only, no backend, no analytics; the build **refuses** to write a page that would make a network request. Line labels are extracted from the official IRS PDF per year (`evidence/`) because the IRS **swapped lines 27a and 27b in 2025**. Read `DECISIONS.md` first. Source lives here; **two** pages are **generated** into `website/` — the tool at `tools/schedule-c-profit-and-loss/` and the line-by-line guide at `guides/schedule-c-line-by-line/`, whose line numbers are looked up from `years/*.mjs` at build time rather than typed into the prose, for the same 27a/27b reason | JavaScript (ES modules, no dependencies shipped), Python for the copy spec | `cd schedule-c-pl && npm test` (**150**), `npm run build` (both pages), `python3 copy.spec.py` (24), `npm run walk` — drives the built page in a real browser with the network cut off (62) |
