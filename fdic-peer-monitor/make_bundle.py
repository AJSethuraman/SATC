#!/usr/bin/env python3
"""Generate build_fdic_monitor.py -- a single self-contained, ASCII-only
...
It writes into the current folder:
    ...
    Bank_Peer_Monitor_fallback.xlsx (no VBA -- if your Excel rejects the
...
def build():
    ...
    L.append('"""One-file builder for the Bank Counterparty & Peer Monitor (FDIC).')
    L.append('')
    ...
    L.append('    python build_fdic_monitor.py')
    L.append('')
    ...
    L.append('                                                       # no API key needed')
    L.append('    python runner.py --lookup "Frost Bank"             # find a CERT to add')
    ...
    L.append('    print("Live FDIC pulls are KEYLESS -- no API key or account needed.")')
    L.append('')
