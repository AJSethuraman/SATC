#!/usr/bin/env python3
"""Generate build_edgar_tracker.py -- a single self-contained, ASCII-only
...
It writes into the current folder:
    ...
    Crit_Class_Tracker_fallback.xlsx (no VBA -- if your Excel rejects the
                                      embedded project, open this, paste
                                      macro.bas into the VBA editor, save
                                      as .xlsm)
...
MODULES = ("keybank_style", "member_seed", "runner", "provenance_seed",
           "vba_writer", "assemble_xlsm", "build_workbook")
OUT_NAME = "build_edgar_tracker.py"
WB = "Crit_Class_Tracker"
...
def build():
    ...
    L.append('"""One-file builder for the EDGAR Criticized/Classified Tracker.')
    L.append('')
    ...
    L.append('    python build_edgar_tracker.py')
    L.append('')
    ...
    L.append('    python runner.py --workbook ".\\\\%s.xlsm"           # LIVE -- keyless, but' % WB)
    L.append('        # set edgar_user_agent in _config first ("{org} {email}" -- SEC')
    L.append('        # fair-access requires a declared User-Agent; no API key needed)')
    L.append('    python runner.py --workbook ".\\\\%s.xlsm" --lookup "Frost"   # find a CIK' % WB)
    L.append('    python runner.py --workbook ".\\\\%s.xlsm" --tieout 35527     # verify a feed' % WB)
    L.append('    python runner.py --workbook ".\\\\%s.xlsm" --selftest        # proxy check' % WB)
    L.append('Peers live in the _config [PEERS] tab: edit a row + re-run, no rebuild.')
    L.append('Or drop control_center.py next to it and run: python control_center.py')
    L.append('"""')
    ...
    L.append('    print("Live EDGAR pulls are KEYLESS but need edgar_user_agent set in")')
    L.append('    print("_config (SEC fair-access User-Agent). See RUN.txt after Extract.")')
    L.append('')
