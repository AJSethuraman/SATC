Attribute VB_Name = "CritClassTracker"
'================================================================================
' EDGAR Criticized/Classified Tracker -- extractor macro
' (BUILD_SPEC_EDGAR.md Section 5: EXTRACT-ONLY).
'
...
Private Const STATUS_SHEET As String = "Dashboard_Criticized"
...
Public Sub ExtractFiles()
    ...
    MsgBox "Extracted next to this workbook:" & vbCrLf & _
           ...
           "Live pulls are KEYLESS but REQUIRE edgar_user_agent set in _config " & _
           "(SEC fair-access: ""{org} {email}"").", _
...
Private Function RunText() As String
    ...
    RunText = _
        "EDGAR Criticized/Classified Tracker -- run from PowerShell (workbook must be CLOSED)" & nl & nl & _
        "1) Install dependencies (once):" & nl & _
        "   python -m pip install -r requirements.txt" & nl & nl & _
        "2) Try-the-button demo data (offline, deterministic, no User-Agent needed):" & nl & _
        "   python .\runner.py --workbook " & q & ".\" & wb & q & " --demo" & nl & nl & _
        "3) Live data (SEC EDGAR -- KEYLESS, no API key or account, BUT you must first" & nl & _
        "   set edgar_user_agent in the _config tab to " & q & "{your org} {your email}" & q & ";" & nl & _
        "   SEC fair-access policy requires a declared User-Agent on every request):" & nl & _
        "   python .\runner.py --workbook " & q & ".\" & wb & q & nl & nl & _
        "4) Find a company's CIK to add to the _config [PEERS] table:" & nl & _
        "   python .\runner.py --workbook " & q & ".\" & wb & q & " --lookup " & q & "Frost" & q & nl & nl & _
        "5) Tie a bank-quarter out against the filed 10-Q (value + members + accession + URL):" & nl & _
        "   python .\runner.py --workbook " & q & ".\" & wb & q & " --tieout 35527 2026-03-31" & nl & nl & _
        "6) Connectivity self-test (corporate proxies often block sec.gov):" & nl & _
        "   python .\runner.py --workbook " & q & ".\" & wb & q & " --selftest" & nl & nl & _
        "The peer list is the _config [PEERS] table: slot | cik | name | ticker | active." & nl & _
        "Add a bank = fill a free slot row + re-run (no rebuild). Remove = active FALSE." & nl & _
        "More banks than built slots and the runner tells you the exact rebuild command." & nl & _
        "New disclosure members bootstrap into [MEMBER_MAP] as unmapped -- map by hand, re-run." & nl & nl & _
