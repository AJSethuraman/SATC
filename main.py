...
def truncate_path(path: str, max_len: int = 110) -> str:
    if len(path) <= max_len:
        return path
    keep = max_len - 3
    head = keep // 2
...
class FileReviewScannerApp:
    def __init__(self, root: tk.Tk) -> None:
        ...
        self.discovered_var = tk.StringVar(value="0")
        ...
        self.recent_path_var = tk.StringVar(value="")
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Checkbutton(frame, text="Identify exact duplicates using SHA-256 hashes", variable=self.duplicates_var).grid(row=5, column=0, columnspan=2, sticky="w", pady=(12, 0))

        controls = ttk.Frame(frame)
        controls.grid(row=6, column=0, columnspan=2, sticky="w", pady=(14, 0))
        self.start_button = ttk.Button(controls, text="Start Scan", command=self._start_scan)
        self.start_button.pack(side=tk.LEFT)
        self.cancel_button = ttk.Button(controls, text="Cancel Scan", command=self._cancel_scan, state=tk.DISABLED)
        ...
        rows = [("Current Phase", self.phase_var), ("Elapsed", self.elapsed_var), ("Files Discovered", self.discovered_var), ("Files Included", self.files_scanned_var), ("Files Hashed", self.files_hashed_var), ("Duplicate Groups", self.duplicate_groups_var), ("Errors", self.errors_var), ("Recent Path", self.recent_path_var)]
        ...
        self.status_text = tk.Text(status, height=10, wrap=tk.WORD)
        self.status_text.grid(row=10, column=0, columnspan=2, sticky="nsew", pady=(8, 0))
        ...
        status.rowconfigure(10, weight=1)
    ...
    def _pick_output_file(self) -> None:
        p = filedialog.asksaveasfilename(title="Select Output Excel Workbook", defaultextension=".xlsx", filetypes=[("Excel Workbook", "*.xlsx")], initialfile="FileReviewScanner_Report.xlsx")
        if p:
    ...
    def _start_scan(self) -> None:
        ...
        config = ScanConfig(root_path=scan_root, output_file=output_file, filter_mode=mode, extension_text=self.extensions_var.get(), detect_duplicates=self.duplicates_var.get())
        self.scan_thread = threading.Thread(target=run_scan, args=(config, self.events, self.cancel_event), daemon=True)
        ...
        try:
            while True:
                ...
                if etype == "progress":
                    ...
                    self.elapsed_var.set(f"{event.get('elapsed_seconds', 0)}s")
                    self.discovered_var.set(str(event.get("files_discovered", self.discovered_var.get())))
                    ...
                    self.recent_path_var.set(truncate_path(event.get("recent_path", "")))
