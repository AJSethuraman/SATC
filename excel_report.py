...
def build_workbook(records: list[dict[str, Any]], errors: list[dict[str, str]], scan_meta: dict[str, Any], output_path: Path, progress_cb: Callable[[str], None] | None = None) -> None:
    if progress_cb:
        progress_cb("Creating workbook structure...")
    wb = Workbook()
    ws_review = wb.active
    ...
    review_cols = ["Client Decision", "Recommended Action", "Duplicate Group ID", "Duplicate Confidence", "Full Path", "Folder Path", "File Name", "Extension", "Category", "Size Bytes", "Size MB", "Size GB", "Created Date", "Modified Date", "Last Accessed Date", "Top-Level Folder", "Parent Folder", "Likely Source / Machine", "Hash", "Reason Flagged", "Notes", "Scan Error"]
    ws_review.append(review_cols)
    for r in records:
        ws_review.append(["", r["recommended_action"], r["duplicate_group_id"], r["duplicate_confidence"], r["full_path"], r["folder_path"], r["file_name"], r["extension"], r["category"], r["size_bytes"], r["size_mb"], r["size_gb"], _fmt_dt(r["created_date"]), _fmt_dt(r["modified_date"]), _fmt_dt(r["accessed_date"]), r["top_level_folder"], r["parent_folder"], r["likely_source"], r.get("hash", ""), r["reason_flagged"], "", r.get("scan_error", "")])
    ...
    if progress_cb:
        progress_cb("Building duplicate groups sheet...")
    ws_dups.append(["Duplicate Group ID", "File Count", "Total Group Size GB", "Potential Recoverable Size GB", "File Name Examples", "Categories", "Oldest Modified Date", "Newest Modified Date", "Locations", "Hash"])
    ...
    for group_id in sorted(dup_groups):
        ...
        modified_vals = [x["modified_date"] for x in rows if x["modified_date"]]
        ws_dups.append([group_id, len(rows), round(total / (1024**3), 2), round(recoverable / (1024**3), 2), ", ".join(sorted({x["file_name"] for x in rows})[:5]), ", ".join(sorted({x["category"] for x in rows})), _fmt_dt(min(modified_vals) if modified_vals else None), _fmt_dt(max(modified_vals) if modified_vals else None), " | ".join(sorted({x["folder_path"] for x in rows})[:5]), rows[0].get("hash", "")])

    if progress_cb:
        progress_cb("Building summary sheet...")
    ...
    items = [
        ...
        ("Total Duplicate Files", scan_meta["total_duplicate_files"]), ("Potential Recoverable Duplicate Size GB", scan_meta["potential_recoverable_gb"]),
        ("Files Skipped Due To Error", scan_meta["files_skipped_error"]), ("Largest File Size GB", scan_meta["largest_file_gb"]),
        ("Oldest Modified Date", scan_meta["oldest_modified"]), ("Newest Modified Date", scan_meta["newest_modified"]),
    ]
    for k, v in items:
        ws_sum.append([k, v])
    ws_sum.append([])
    ...
    if progress_cb:
        progress_cb("Saving workbook...")
    wb.save(output_path)
    if progress_cb:
        progress_cb("Workbook saved.")
