#!/usr/bin/env python3
"""Generate build_macro_monitor.py -- a single self-contained, ASCII-only
...
It writes into the current folder:
    ...
    Macro_Early_Warning_Monitor_fallback.xlsx (no VBA -- if your Excel rejects
                                               the embedded project, open this,
                                               paste macro.bas into the VBA
                                               editor, save as .xlsm)
...
OUT_NAME = "build_macro_monitor.py"
WB = "Macro_Early_Warning_Monitor"
...
def build():
    ...
    L.append('"""One-file builder for the Macro Early-Warning Monitor (FRED).')
    L.append('')
    ...
    L.append('    python build_macro_monitor.py')
    L.append('')
    ...
    L.append('    $env:FRED_API_KEY = "yourkey"                       # live FRED needs a key')
    L.append('    python runner.py --workbook ".\\\\%s.xlsm"           # live FRED' % WB)
    ...
    L.append('    print("Live FRED runs need a key: $env:FRED_API_KEY = \\"yourkey\\".")')
    L.append('')
