Attribute VB_Name = "MacroMonitor"
'================================================================================
' Macro Early-Warning Monitor (FRED) -- extractor macro
' (BUILD_SPEC_MACRO.md Section 5: EXTRACT-ONLY).
'
...
Private Const STATUS_SHEET As String = "Dashboard_Conditions"
...
Private Function RunText() As String
    ...
    RunText = _
        "Macro Early-Warning Monitor -- run from PowerShell (workbook must be CLOSED)" & nl & nl & _
        ...
        "3) Live data (FRED REST API -- needs a free API key):" & nl & _
        "   $env:FRED_API_KEY = " & q & "yourkey" & q & nl & _
        "   python .\runner.py --workbook " & q & ".\" & wb & q & nl & nl & _
        "The key comes from the FRED_API_KEY env var (preferred) or the _config" & nl & _
        "fred_api_key cell -- never hardcoded, never echoed. Absent key = fail fast." & nl & nl & _
...
' All three dashboards read from the single Raw_FRED tab.
Public Sub PaintSparklines()
    On Error Resume Next
    PaintLaneSparklines "Dashboard_Conditions"
    PaintLaneSparklines "Dashboard_Labor"
    PaintLaneSparklines "Dashboard_Housing_Sentiment"
...
Private Sub PaintLaneSparklines(dashName As String)
    ...
    Const RAW_NAME As String = "Raw_FRED"
