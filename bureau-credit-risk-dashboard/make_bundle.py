#!/usr/bin/env python3
"""Generate build_bureau_monitor.py -- a single self-contained, ASCII-only
...
It writes into the current folder:
    ...
    Consumer_Credit_Risk_Monitor_fallback.xlsx (no VBA -- if your Excel rejects
...
OUT_NAME = "build_bureau_monitor.py"
WB = "Consumer_Credit_Risk_Monitor"
...
def build():
    ...
    L.append('"""One-file builder for the Consumer Credit-Risk Monitor (bureau feed).')
    L.append('')
    ...
    L.append('    python build_bureau_monitor.py')
    L.append('')
    ...
    L.append('    python runner.py --workbook ".\\\\%s.xlsm"           # live HHDC' % WB)
