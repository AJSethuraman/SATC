...
Run 2026-09-20 (as-of 2026-06-30), 40,000 loans per book, generator 0.1.0. Every command below was run against `build_pack.py`, the same pure-ASCII script a desk receives, from an empty folder, and every answer was read back out of the workbook the way Excel reads it. Generated by `tools/exercise.py`; do not edit by hand.
...
```
python build_pack.py --data effect.csv --asof 2026-06-30 --run-date 2026-09-20 --config effect.yaml -o effect.xlsx
```
...
- 29,343 seasoned loans, 257 events; 10,657 too young to count. Built in 3.8 s.
...
```
python build_pack.py --data null.csv --asof 2026-06-30 --run-date 2026-09-20 --config null.yaml -o null.xlsx
```
...
```
python build_pack.py --data confounded.csv --asof 2026-06-30 --run-date 2026-09-20 --config confounded.yaml -o confounded.xlsx
```
...
```
python build_pack.py --data effect.csv --asof 2026-06-30 --run-date 2026-09-20 --config flag.yaml -o flag.xlsx
```
...
```
python build_pack.py --data effect.csv --asof 2026-06-30 --run-date 2026-09-20 --config snapshot.yaml -o snapshot.xlsx
```
...
python build_pack.py --data extract.csv --asof 2026-06-30 --run-date 2026-09-20 --config filled.yaml -o desk.xlsx
```

**What the pack said.**

- Refusal, first lines: question.yaml: 20 values still to fill in (written by `pack init`): | `name` still reads [CONFIRM: a short name for this question, letters and underscores] — replace it with your answer | `population.loan_id` still reads [CONFIRM: the column that numbers the loans] — replace it with your answer
...
- SHA-256 of the desk build: 3b51561c74e382c3c3646b9c97bf044eac5fa26c43b7ffb4630b8d4be77ff65f

**Checks: 5 of 5 agree.**

- ✔ the script wrote a question file listing every column — 12 columns, 20 values to fill in
- ✔ the unfilled file is refused, naming all 20 slots — exit 2, 21 lines
- ✔ the filled file validates — 29,343 seasoned
- ✔ the filled file builds
- ✔ the desk's workbook is byte-identical to one built here from the same inputs — desk 3b51561c74e382c3… here 3b51561c74e382c3…
...
```
python build_pack.py --data dirty.csv --asof 2026-06-30 --run-date 2026-09-20 --config effect.yaml -o dirty.xlsx
python build_pack.py --data ambiguous.csv --asof 2026-06-30 --run-date 2026-09-20 --config nodate.yaml -o ambiguous.xlsx
...
```
python build_pack.py --data blanks.csv --asof 2026-06-30 --run-date 2026-09-20 --config effect.yaml -o blanks.xlsx
```
...
```
python build_pack.py --data effect.csv --asof 2026-06-30 --run-date 2026-09-20 --config effect.yaml -o effect-again.xlsx
python build_pack.py --data effect.csv --asof 2026-06-30 --run-date 2026-09-19 --config effect.yaml -o effect-other-day.xlsx
```

**What the pack said.**

- SHA-256, built twice: 6c6926ec8a721ad174d14ed7df105511fc6db344a8bda13ee372cd726b0642ec

**Checks: 2 of 2 agree.**

- ✔ built again: identical SHA-256 — 6c6926ec8a721ad1… vs 6c6926ec8a721ad1…
- ✔ a different run date: a different file — af81d34fde870824…
...
```
python build_pack.py --data large.csv --asof 2026-06-30 --run-date 2026-09-20 --config large.yaml -o large.xlsx
```

**What the pack said.**

- 73,293 seasoned loans, 649 events, built in 9.6 s; workbook 162 KB against 161 KB at 40,000 loans.

**Checks: 4 of 4 agree.**

- ✔ the script built the pack and exited 0
- ✔ built in under a minute — 9.6 s
- ✔ the workbook is no larger than the 40,000-loan one by more than a tenth — 165,378 vs 164,842 bytes
